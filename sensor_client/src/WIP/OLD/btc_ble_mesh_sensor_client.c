...
{
    esp_mesh_sensor_client_cb_t btc_mesh_cb = (esp_mesh_sensor_client_cb_t)btc_profile_cb_get(BTC_PID_SENSOR_CLIENT);
    if (btc_mesh_cb) {
...
void bt_mesh_callback_sensor_status_to_btc(uint32_t opcode, uint8_t evt_type,
...
void btc_mesh_sensor_client_publish_callback(uint32_t opcode, struct bt_mesh_model *model,
