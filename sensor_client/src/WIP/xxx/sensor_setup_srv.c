...
printk("Prop ID: 0x%04x \t",sensor_setting_property_id);
	...
	// int attr_cnt = sizeof(sensor_attr_list)/sizeof(struct bt_mesh_sensor_setting_attr);


	// printk("Attr Count: %d \t",attr_cnt);
	// if (attr_cnt <=1){
	// 	attr_cnt++;
	// 	sensor_attr_list = malloc(sizeof(struct bt_mesh_sensor_setting_attr) * attr_cnt);
	// 	// int attr_cnt = sizeof(sensor_attr_list)/sizeof(sensor_attr_list[0]);
	// 	printk("malloc Count: %d \t",attr_cnt);
	// }
...
printk("Slot: %d \n", attr_cnt);
	...
	if (added < 0){
		int newsize = attr_cnt + 1;
		// realloc(sensor_attr_list, newsize);

		if(!realloc(sensor_attr_list, newsize)){
