...
void st_hts221_get_temp(struct sensor_value* temp)
{
	...
	if (st_hts221 == NULL) {
		printf("ERROR: Could not read HTS221 device\n");
		return;
	}
	...
	k_sem_take(&st_hts221_sem, K_NO_WAIT);
	...
	if(sensor_channel_get(st_hts221, SENSOR_CHAN_AMBIENT_TEMP, temp)){
		...
		printf("Temperature:%.3f C (st_hts221)\n", sensor_value_to_double(temp));
	}
...
hum->val1 = 92;
hum->val2 = 99000;
		printf("ERROR: Sensor HTS221 sample update error\n");
		return;
	}
	...
	if(sensor_channel_get(st_hts221, SENSOR_CHAN_HUMIDITY, hum)){
hum->val1 = 93;		
		printf("ERROR: st_hts221 humidity channel failed\n");
		return;
	} else {
		// printf("st_hts221 Humidity Accessed: %3.3f  \n",
