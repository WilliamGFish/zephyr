...
restart:
	...
	if (ret < 0) {
		...
		// ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
		// 		     NULL, 0, "AT+UMNOPROF=100", &mdata.sem_response,
		// 		     MDM_CMD_TIMEOUT);
		// ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
		// 		     NULL, 0, "AT+CFUN=15", &mdata.sem_response,
		// 		     MDM_CMD_TIMEOUT);

		// ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
		// 		     NULL, 0, "AT+URAT=7", &mdata.sem_response,
		// 		     MDM_CMD_TIMEOUT);

		// // // ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
		// // // 		     NULL, 0, "AT+UBANDMASK=0,0", &mdata.sem_response,
		// // // 		     MDM_CMD_TIMEOUT);
		...
		ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
				     NULL, 0, "AT+UMNOPROF?", &mdata.sem_response,
				     MDM_CMD_TIMEOUT);

		ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
				     NULL, 0, "AT+UBANDMASK?", &mdata.sem_response,
				     MDM_CMD_TIMEOUT);

		ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
				     NULL, 0, "AT+URAT?", &mdata.sem_response,
				     MDM_CMD_TIMEOUT);
	...
	counter = 0;
	while (counter++ < 10000 && mdata.ev_creg != 1 && mdata.ev_creg != 5) {

if (counter % 10 == 0) {
			ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
				NULL, 0, "AT+CEREG?", &mdata.sem_response,
				MDM_CMD_TIMEOUT);
}
		// if (counter == 20) {
...
static const struct modem_cmd unsol_cmds[] = {
	...
	MODEM_CMD("+CREG: ", on_cmd_socknotifycreg, 0U, ""),
	MODEM_CMD("+CEREG: ", on_cmd_socknotifycereg, 2U, ","),
};
