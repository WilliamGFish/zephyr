				...
				k_timeout_t timeout)
{
	...
	LOG_INF("SEND_SOCKET_DATA.........%s:%d", log_strdup(send_buf), buf_len);
	ret = modem_cmd_send_nolock(&mctx.iface, &mctx.cmd_handler,
				    NULL, 0U, send_buf, NULL, K_NO_WAIT);
	if (ret < 0) {
			LOG_INF("modem_cmd_send_nolock.........ERROR:%d",ret);
		goto exit;
	}
	LOG_INF("SEND_SOCKET_DATA.........2");
...
MODEM_CMD_DIRECT_DEFINE(on_prompt)
{
 ...
 * example +UULOC: 13/04/2011,09:54:51.000,log_strdup(,-1.5420349,49,1  **/
MODEM_CMD_DEFINE(on_cmd_sockread_localization){

	// // mctx.data_localization.date = ATOI(argv[1], 0, "date");
	// memcpy(mctx.data_localization.date, argv[0], strlen(argv[0]));
	// // strcpy(mctx.data_localization.date, argv[0]);
	// // mctx.data_localization.time = ATOI(argv[2], 0, "time");
	// memcpy(mctx.data_localization.time, argv[1], strlen(argv[1]));
	// // mctx.data_localization.lat = ATOI(argv[3], 0, "latitude");
	// memcpy(mctx.data_localization.lat, argv[2], strlen(argv[2]));
	// // mctx.data_localization.lon = ATOI(argv[4], 0, "longitude");
	// memcpy(mctx.data_localization.lon, argv[3], strlen(argv[3]));
	// mctx.data_localization.alt = ATOI(argv[4], 0, "altitude");
	// mctx.data_localization.uncertainty = ATOI(argv[5], 0, "uncertainty");


	/* Convert arguements to variables */
	memcpy(mdata.mdm_date, argv[0], strlen(argv[0]));
	memcpy(mdata.mdm_time, argv[1], strlen(argv[1]));
	memcpy(mdata.mdm_lat, argv[2], strlen(argv[2]));
	memcpy(mdata.mdm_lon, argv[3], strlen(argv[3]));
	memcpy(mdata.mdm_alt, argv[4], strlen(argv[4]));
	// mdata.mdm_alt = ATOI(argv[4], 0, "altitude");
	// mdata.mdm_alt = 10;
	mdata.mdm_uncertainty = ATOI(argv[5], 0, "uncertainty");


	// /* Added for UBlox localization data to modem context data */
	// memset(mdata.mdm_date, '\0', sizeof(mdata.mdm_date));
	// strcpy(mdata.mdm_date, argv[0]);
	// memset(mdata.mdm_time, '\0', sizeof(mdata.mdm_time));
	// strcpy(mdata.mdm_time, argv[1]);
	// memset(mdata.mdm_lat, '\0', sizeof(mdata.mdm_lat));
	// strcpy(mdata.mdm_lat, argv[2]);
	// memset(mdata.mdm_lon, '\0', sizeof(mdata.mdm_lon));
	// strcpy(mdata.mdm_lon, argv[3]);
	// mdata.mdm_alt = ATOI(argv[4], 0, "altitude");
	// mdata.mdm_uncertainty = ATOI(argv[5], 0, "uncertainty");

	LOG_INF("mdata values:");
	LOG_INF("date %s", log_strdup(mdata.mdm_date));
	LOG_INF("time %s", log_strdup(mdata.mdm_time));
	LOG_INF("lat %s", log_strdup(mdata.mdm_lat));
	LOG_INF("lon %s", log_strdup(mdata.mdm_lon));
	LOG_INF("alt %s", log_strdup(mdata.mdm_alt));
	LOG_INF("uncertainty %d", mdata.mdm_uncertainty);
	...
	mctx.data_localization.uncertainty = mdata.mdm_uncertainty;



	// // /* Added for UBlox localization data to modem context data */
	// // memset(mctx.data_localization.date, '\0', sizeof(mctx.data_localization.date));
	// memset(mctx.data_localization.date, '\0', sizeof(mdata.mdm_date));
	// strcpy(mctx.data_localization.date, argv[0]);
	// memset(mctx.data_localization.time, '\0', sizeof(mdata.mdm_time));
	// strcpy(mctx.data_localization.time, argv[1]);
	// memset(mctx.data_localization.lat, '\0', sizeof(mdata.mdm_lat));
	// strcpy(mctx.data_localization.lat, argv[2]);

	// memset(mctx.data_localization.time, '\0', sizeof(mctx.data_localization.time));
	// strcpy(mctx.data_localization.time, mdata.mdm_time);

	// memset(mctx.data_localization.lat, '\0', sizeof(mctx.data_localization.lat));
	// strcpy(mctx.data_localization.lat, mdata.mdm_lat);
	// memset(mctx.data_localization.lon, '\0', sizeof(mctx.data_localization.lon));
	// strcpy(mctx.data_localization.lon, mdata.mdm_lon);
	// mctx.data_localization.alt = ATOI(argv[4], 0, "altitude");
	// mctx.data_localization.uncertainty = ATOI(argv[5], 0, "uncertainty");


	LOG_INF("mctx.data_localization values:");
	LOG_INF("date %s", log_strdup(mctx.data_localization.date));
	LOG_INF("time %s", log_strdup(mctx.data_localization.time));
	LOG_INF("lat %s", log_strdup(mctx.data_localization.lat));
	LOG_INF("lon %s", log_strdup(mctx.data_localization.lon));
	LOG_INF("alt %s", log_strdup(mctx.data_localization.alt));
	LOG_INF("uncertainty %d", mctx.data_localization.uncertainty);
...
restart:
	...
	/* configure UBlox LOCATION FINDER service */
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
			     NULL, 0, "AT+UGSRV=\"cell-live1.services.u-blox.com\",\"cell-live2.services.u-blox.com\",\"J-aI_-ZUNEef7ZGUv67LJw\",14,4,1,65,0,15",
			     &mdata.sem_response,
			     MDM_CMD_TIMEOUT);
	if (ret < 0) {
		LOG_ERR("Failed to invoke localization data (UBlox)");
...
static const struct modem_cmd unsol_cmds[] = {
	...
	MODEM_CMD("+UULOC: ", on_cmd_sockread_localization, 6U, ","),	
};
