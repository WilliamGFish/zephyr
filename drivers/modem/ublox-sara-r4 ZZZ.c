...
MODEM_CMD_DEFINE(on_cmd_sockread_localization){
	...
	memcpy(mctx.data_localization.date, argv[1], strlen(argv[1]));
	// mctx.data_localization.time = ATOI(argv[2], 0, "time");
	memcpy(mctx.data_localization.time, argv[2], strlen(argv[2]));
	// mctx.data_localization.lat = ATOI(argv[3], 0, "latitude");
	memcpy(mctx.data_localization.lat, argv[3], strlen(argv[3]));
	// mctx.data_localization.lon = ATOI(argv[4], 0, "longitude");
	memcpy(mctx.data_localization.lon, argv[4], strlen(argv[4]));
	mctx.data_localization.alt = ATOI(argv[5], 0, "altitude");
	mctx.data_localization.uncertainty = ATOI(argv[6], 0, "uncertainty");

	LOG_INF("date %s", mctx.data_localization.date);
	LOG_INF("time %s", mctx.data_localization.time);
	LOG_INF("lat %s", mctx.data_localization.lat);
	LOG_INF("lon %s", mctx.data_localization.lon);
	LOG_INF("alt %d", mctx.data_localization.alt);
	...
	// mctx.data_localization.uncertainty = mdata.mdm_loc_uncertainty;
...
/* External Call to return Cell Location */
