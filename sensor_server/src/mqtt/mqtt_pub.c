...
static int try_to_connect(struct mqtt_client *client)
{
	...
	printk("TRYING TO CONNECT TO MQTT...............");
...
restart:
	...
	while (!connected){
		...
		rc = process_mqtt_and_sleep(&client_ctx, APP_SLEEP_MSECS);
		PRINT_RESULT("process_mqtt_and_sleep", rc);
	}
	...
	if (rc != 0){
		PRINT_RESULT("ERROR: Publish Failed !", rc);
		k_sleep(K_MSEC(1000));
		goto restart;
	}
	...
	// printk("mqtt_data_t %d tx_data: %s \n", rc, mqttbuf);
...
void fifo_loop(void){
	int err, count = 0U, badcount =0U, failcount = 0U;
	char sent_message[MQTT_MSG_SIZE] = " "; // ORIG 300
	char empty_message[MQTT_MSG_SIZE] = " ";
	struct mqtt_data_t *tx_data;

	memcpy(tx_data->message, empty_message, MQTT_MSG_SIZE);

	// tx_data = k_fifo_get(&mqtt_fifo, K_NO_WAIT); //K_NO_WAIT K_FOREVER

	// printk("MQTT fifo_loop..............................\n");

	// printk("attempting to connect: \n");
	// err = try_to_connect(&client_ctx);
	// PRINT_RESULT("try_to_connect", err);
	// // SUCCESS_OR_EXIT(err);

	// err = mqtt_ping(&client_ctx);
	// PRINT_RESULT("mqtt_ping", err);

	// err = mqtt_live(&client_ctx);
	// if (err != 0) {
	// 	PRINT_RESULT("mqtt_live", err);
	// }

	printk(" -------------- STARTING MQTT BUFFER LOOP -------------- \n");


	err = k_fifo_is_empty(&mqtt_fifo);
	if (!err){
		printk("Messsages in buffer: %d \n\n\n\n\n", err);
	} else
	{
		printk("No mesaages in buffer\n");
	}
	...
	while (1) {
		...
		// k_sleep(K_MSEC(2000)); // 2700 Slow down sending for Adafruit throttle
		// if (err){
		// 	k_free(tx_data);
		// }else
		// {
...
K_THREAD_DEFINE(mqtt_out_id, STACKSIZE, fifo_loop, NULL, NULL, NULL, PRIORITY,
		0, THREAD_DELAY);	