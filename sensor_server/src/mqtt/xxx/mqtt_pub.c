...
void fifo_loop(void){
	int err, count = 0U;
	char sent_message[500];
	char empty_message[10] = "";
...
printk(" -------------- STARTING MQTT BUFFER LOOP -------------- \n");
	...
	while (1) {
		...
		if (err != 0){
			...
			strcpy(sent_message, empty_message);
			...
			// printk("Messsages the same: %s = %s  \n", sent_message, tx_data->message);
		}
		
		
		// k_sleep(K_MSEC(5000)); // 2700 Slow down sending for Adafruit throttle
		// if (err){
		// 	k_free(tx_data);
		// }else
		// {
