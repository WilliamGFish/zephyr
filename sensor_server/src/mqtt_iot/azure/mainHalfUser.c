...
#if Z_LIBC_PARTITION_EXISTS
		printk("z_libc_partition setup...");
#else
		printk("app_partition setup...");
#endif
...
printk("Starting user_thread...!!");
	k_thread_start(app_thread);
	k_thread_join(app_thread, K_FOREVER);
	// k_thread_join(app_thread, K_NO_WAIT);
#else
printk("Starting fifo_loop...!!");
	exit(fifo_loop());
#endif	
}
