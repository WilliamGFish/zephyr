...
static const struct mdm_control_pinconfig pinconfig[] = {
	/* MDM_POWER */
	PINCONFIG(DT_UBLOX_SARA_R4_0_MDM_POWER_GPIOS_GPIO_CONTROLLER,
		  DT_UBLOX_SARA_R4_0_MDM_POWER_GPIOS_GPIO_PIN),

	/* MDM_RESET */
	PINCONFIG(DT_UBLOX_SARA_R4_0_MDM_RESET_GPIOS_GPIO_CONTROLLER,
		  DT_UBLOX_SARA_R4_0_MDM_RESET_GPIOS_GPIO_PIN),
};
...
static inline void _hexdump(const uint8_t *packet, size_t length)
{
...
static void on_cmd_sockcreate(struct net_buf **buf, uint16_t len)
{
	char value[2];
	struct modem_socket *sock = NULL;
				   ...
				   uint16_t len)
{
	...
	hdr_len = pkt_setup_ip_data(sock->recv_pkt, sock, &sock->src);
 ...
 *          "<hex_data>"
*/
static void on_cmd_sockread_udp(struct net_buf **buf, uint16_t len)
{
	int socket_id;
	char value[2];
...
static void on_cmd_sockread_tcp(struct net_buf **buf, uint16_t len)
{
	int socket_id;
	char value[2];
...
static void on_cmd_socknotifyclose(struct net_buf **buf, uint16_t len)
{
	char value[2];
	int socket_id;
...
static void on_cmd_socknotifydata(struct net_buf **buf, uint16_t len)
{
...
static void on_cmd_socknotifycreg(struct net_buf **buf, uint16_t len)
{
	char value[8];
	size_t out_len;
...
static inline struct net_buf *read_rx_allocator(int32_t timeout, void *user_data)
{
...
restart:
	...
	ret = send_at_cmd(NULL, "AT+CGDCONT=1,\"IP\",\""
				CONFIG_MODEM_UBLOX_SARA_R4_OPERATOR "\"",
			  MDM_CMD_TIMEOUT);
	if (ret < 0) {
