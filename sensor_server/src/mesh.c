...
static void heartbeat_sub(const struct bt_mesh_hb_sub *sub, uint8_t hops,
		      uint16_t feat)
{
	board_heartbeat(hops, feat);
	printk("**************************************************\n");
...
int format_marshalled_data(struct bt_mesh_sensor_property_value sensor_data, int property_count)
{
	/* Data message buffer */
	int i = 0U;
	char lat[16] = "53.992119", lon[16] = "-1.541812", alt[16] = "0.0";
	char time_str[sizeof("1970-01-01T00:00:00Z")];
	struct tm now_tm;


	/* Shows the data recieved from node */
	// LOG_DBG("NodeID: %04x ", sensor_data.node_id);
	// LOG_DBG("PropertyID: %04x ", sensor_data.property_id);
	// LOG_DBG("Count: %d ", property_count);
	...
	mqttcx += snprintf(mqttbuf + mqttcx, 150, "{\"nodeID\":%x,\"%04x\":", sensor_data.node_id, sensor_data.property_id );
#endif
	...
	while (i < property_count) {
		...
		if (property_count == 1) {
			/* Add */
			mqttcx += snprintf(mqttbuf + mqttcx, 50,"%.3f,", sensor_float);
		} else if (i == (property_count -1) ){
			mqttcx += snprintf(mqttbuf + mqttcx, 50,"\"v%d\":%.3f},", i,sensor_float);
			// mqttcx += snprintf(mqttbuf + mqttcx, 15,  "%.3f},",sensor_float);
			// mqttcx += snprintf(mqttbuf + mqttcx, 15,  "%.3f\",",sensor_float);			
		}else if (i == 0U){
			mqttcx += snprintf(mqttbuf + mqttcx, 50,  "{\"v%d\":%.3f,",i,sensor_float);	
			// mqttcx += snprintf(mqttbuf + mqttcx, 15,  "\"%.3f,",sensor_float);	
		...
		static struct modem_context *modem_ctx;

		/* Display Modem context information - struct in modem_receiver.h */
		LOG_DBG("Modem receiver:\n");

		modem_ctx = modem_context_from_id(0U);
		if (modem_ctx) {
			strcpy(lat, modem_ctx->data_localization.lat);
			strcpy(lon, modem_ctx->data_localization.lon);
			strcpy(alt, modem_ctx->data_localization.alt);
			// modem_ctx->data_localization.uncertainty

			char str_time[30];
			snprintf(str_time, 20, "%s %s",
				 modem_ctx->data_localization.date,
				 modem_ctx->data_localization.time);

			LOG_DBG("MDM time:%s\n",
			       modem_ctx->data_localization.time);
			
			// 13/04/2011,09:54:51.000
			char *s = strptime(str_time, "%d/%m/%Y %H:%M:%S", &now_tm);

			if (s == NULL) {
				printf("Cannot parse date"); 
			}
			strftime(time_str, sizeof(time_str), "%FT%TZ", &now_tm);		 
		}

	// LOG_INF("date %s", // LOG_strdup(modem_ctx.context..data_localization.date));
	// LOG_INF("time %s", // LOG_strdup(modem_ctx.context..data_localization.time));
	// LOG_INF("lat %s", // LOG_strdup(modem_ctx.context.ctx.data_localization.lat));
	// LOG_INF("lon %s", // LOG_strdup(modem_ctx.context..data_localization.lon));
	// LOG_INF("alt %s", // LOG_strdup(modem_ctx.context..data_localization.alt));
	// LOG_INF("uncertainty %d", modem_ctx.context..data_localization.uncertainty);
	...
	// mqttcx +=
	// 	snprintf(mqttbuf + mqttcx, 150, "lat:%s,lon:%s,ele:%s,tp:%s}}",
	// 		 lat, lon, alt, time_str);

	snprintf(mqttbuf + mqttcx, 150, 
	"\"location\":{\"lat\":%s,\"lng\":%s,\"ele\":%s},\"ts\":\"%s\"}",
	lat, lon, alt, time_str);
	...
	if ((mqttcx) > MIN_LENGTH){
		...
		strncpy(msg_data.message, mqttbuf, sizeof(msg_data.message));	
		len = strlen(mqttbuf);

		// LOG_DBG("\n Added to buffer %s:%d \n", msg_data.message, strlen(msg_data.message));
...
static void prov_node_added(uint16_t net_idx, uint8_t uuid[16], uint16_t addr, uint8_t num_elem)
{
	...
	// LOG_DBG("Remote node provisioned, net_idx 0x%04x address 0x%04x elements %d", net_idx, addr, num_elem);
}

static void prov_complete(uint16_t net_idx, uint16_t addr)
{
	// LOG_DBG("Local node provisioned, net_idx 0x%04x address 0x%04x", net_idx, addr);
}
...
void provisioner_init(void)
{
	...
	while (1) {
		...
		if (err == -EBUSY){
			printk("ERROR: Provisioning failed radio busy:0x%04x (err %d)\n", net_idx, err);
