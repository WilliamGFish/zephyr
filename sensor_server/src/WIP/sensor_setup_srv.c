 ...
 * add new element 
 */
printk("Sent Prop ID: 0x%04x \t", sensor_property_id);
printk("Sent Attr ID: 0x%04x \n",sensor_setting_property_id);


	int added = -1;
	// sensor_attr_list_size++;

	if(sensor_attr_list == NULL) {
		sensor_attr_list = malloc(sizeof(struct bt_mesh_sensor_setting_attr) * sensor_attr_list_size);
	}
	else{
		// sensor_attr_list = realloc(sensor_attr_list, sizeof(*sensor_attr_list) * sensor_attr_list_size);
		sensor_attr_list = realloc(sensor_attr_list, sizeof(struct bt_mesh_sensor_setting_attr) * sensor_attr_list_size);
	}
	...
	for (int i = 0; i < sensor_attr_list_size; i++) {
		...
		if (strcmp(sensor_attr_list[i].dev_name, NULL) ||
			...
			strcpy(sensor_attr_list[i].dev_name,dev_name);


			// memcpy(&sensor_attr_list[i]
			// 	       .sensor_settings.sensor_property_id,
			//        &sensor_property_id, sizeof(sensor_property_id));
			...
			printk("Sent ID: 0x%04x \t", sensor_property_id);			
...
printk("Slot: %d \n", sensor_attr_list_size);
	...
	if (added < 0){
		// int newsize = sensor_attr_list_size + 1;
		// sensor_attr_list = realloc(sensor_attr_list, sizeof(*sensor_attr_list) * newsize);

		sensor_attr_list_size++;
		sensor_attr_list = realloc(sensor_attr_list, sizeof(struct bt_mesh_sensor_setting_attr) * sensor_attr_list_size);


		// if(realloc(sensor_attr_list, sizeof(*sensor_attr_list) * newsize)){
		// 	printk("ERROR: Resize error....!!! \n\n\n");
		// 	return added;
		// };

		// reallocarray(sensor_attr_list, 2, attr_cnt);
		// reallocarray(sensor_attr_list, attr_cnt + 1,
		// 	     sizeof(sensor_attr_list[0]));

		// attr_cnt++;
		// sensor_attr_list[sensor_attr_list_size].dev_name = dev_name;
		strcpy(sensor_attr_list[sensor_attr_list_size].dev_name,dev_name);

		sensor_attr_list[sensor_attr_list_size].sensor_settings.sensor_property_id =
			sensor_property_id;
		sensor_attr_list[sensor_attr_list_size]
			.sensor_settings.sensor_setting_property_id =
			sensor_setting_property_id;
		sensor_attr_list[sensor_attr_list_size]
			.sensor_settings.sensor_setting_access =
			sensor_setting_access;
		sensor_attr_list[sensor_attr_list_size].sensor_settings.sensor_setting_val =
			sensor_setting_val;
		added = 0;		
		printk("INCREASED: \t");		
		printk("Slot: %d \t", sensor_attr_list_size);
		printk("Device: %s\t", sensor_attr_list[sensor_attr_list_size].dev_name);
		printk("Prop ID: 0x%04x \t", sensor_attr_list[sensor_attr_list_size].sensor_settings.sensor_property_id);
		printk("Sent ID: 0x%04x \t", sensor_property_id);			
		printk("Attr ID: 0x%04x \t", sensor_attr_list[sensor_attr_list_size].sensor_settings.sensor_setting_property_id);
		printk("Val1: %d \n", sensor_attr_list[sensor_attr_list_size].sensor_settings.sensor_setting_val.val1);
	}
...
int sensor_setting_attr_update(){
	...
	for (int i = 0; i < sensor_attr_list_size; i++) {
		// if (sensor_attr_list[i].dev_name != ""){
		if (strcmp(sensor_attr_list[i].dev_name, "")) {
     		printk("UPDATING...");
			printk("Slot: %d \t", i);
			printk("Device: %s\t", sensor_attr_list[i].dev_name);
			printk("Prop ID: 0x%04x \t", sensor_attr_list[i].sensor_settings.sensor_property_id);
			      ...
			      struct net_buf_simple *buf)
{
	...
	/** Seporate the 2 divisor and trigger fields **/
	uint8_t xformat = net_buf_simple_pull_u8(buf);  
	...
	if (!srv->cb->sensor_set_cadence) {
		printk("ERROR: No Current sensor_data callback available");
	}else
	{
		if (!srv->cb->sensor_set_cadence(model, cadence_data)){
			printk("ERROR: Failed to set Sensor Cadence via sensor_data callback");
		};
	}	
}
...
**/
	...
	/* Trap bad buffers */
	if (buf->len <= 0) {
		printk("ERROR: No Buffer Data (sensor_settings_get).");
		return;
	}
